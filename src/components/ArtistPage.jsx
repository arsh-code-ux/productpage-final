          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-0">
            ...
            <div className="lg:col-span-3 p-6 md:p-10">
              <motion.div
                ...
                className="glass-card p-8 rounded-2xl shadow-2xl bg-white/60 backdrop-blur-sm border border-white/20"
              >
                <div className="flex items-start gap-6 md:gap-8">
                  <div className="flex-1">
                    <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-900 leading-tight bg-gradient-to-r from-slate-900 via-amber-700 to-slate-900 bg-clip-text text-transparent">{name}</h1>
                    <div className="flex items-center gap-3 mt-3 mb-5">
                      <MapPin className="w-5 h-5 text-slate-600" />
                      <span className="text-slate-700">{location}</span>
                      <span className="ml-3 inline-flex items-center px-3 py-1 rounded-full bg-white/30 text-sm font-medium text-slate-800">70+ Exhibitions</span>
                    </div>

                    <div className="flex flex-wrap gap-2 mb-6">
                      <motion.span whileHover={{ scale: 1.03 }} className="px-3 py-1 rounded-full bg-white/30 text-sm text-slate-800">Geometric Abstraction</motion.span>
                      <motion.span whileHover={{ scale: 1.03 }} className="px-3 py-1 rounded-full bg-white/30 text-sm text-slate-800">Contemporary Art</motion.span>
                      <motion.span whileHover={{ scale: 1.03 }} className="px-3 py-1 rounded-full bg-white/30 text-sm text-slate-800">Mixed Media</motion.span>
                    </div>

                    <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }} className="text-slate-700 leading-relaxed text-md md:text-lg">
                      PRADIP's work blends warm palettes and harmonic geometry to create contemplative compositions. He trained at the British Institute, Mumbai and has exhibited widely since 1995. His pieces are sought by institutional and private collections internationally.
                    </motion.p>
                  </div>

                  <div className="hidden md:flex md:flex-col items-center gap-4 w-44">
                    <div className="w-36 h-36 rounded-2xl overflow-hidden ring-4 ring-white shadow-lg">
                      <img src={image} alt={name} className="w-full h-full object-cover" />
                    </div>
                    <motion.button whileHover={{ scale: 1.03 }} onClick={() => window.open('/contact','_self')} className="px-4 py-2 w-full bg-slate-900 text-white rounded-lg">Contact</motion.button>
                  </div>
                </div>
              </motion.div>
            </div>
